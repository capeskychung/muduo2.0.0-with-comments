                           ...
                           muduo::Timestamp receiveTime)
{
  ...
  while (buffer->readableBytes() >= frameLen)
  {
    ...
    message[1] = receiveTime.microSecondsSinceEpoch(); //服务端收到消息的时间T2
...
void clientConnectionCallback(const TcpConnectionPtr& conn)
{
  ...
  if (conn->connected())
  {
    ...
    conn->setTcpNoDelay(true); //消息一到，立刻发送；不需要等到更多消息到来再发送
  }
  else
  {
...
void sendMyTime()
{
  if (clientConnection)
  {
    ...
    message[0] = Timestamp::now().microSecondsSinceEpoch(); //T1
